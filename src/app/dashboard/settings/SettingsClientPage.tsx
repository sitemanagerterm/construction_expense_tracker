...
type ValidationErrors = {
  ...
  return (
    <div className="w-full">
      ...
      {/* SUBSCRIPTION TAB */}
      {activeTab === "subscription" && (initialUser.role === "OWNER" || initialUser.role === "ADMIN") && (
        <div className="space-y-8 animate-fade-in">
          <div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Subscription & Billing</h2>
            <p className="text-gray-500 dark:text-slate-400 text-sm">Manage your current plan and view payment status.</p>
          </div>
          
          <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-primary-50 dark:bg-accent/5 rounded-bl-full -mr-8 -mt-8 pointer-events-none"></div>
            
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 relative z-10">
              <div>
                <p className="text-sm text-gray-500 dark:text-slate-400 font-semibold uppercase tracking-wider mb-2">Current Plan</p>
                <div className="flex items-center gap-3">
                  <h3 className="text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tight">
                    {plan === 'FREE' ? 'Free Plan' : plan}
                  </h3>
                  {plan !== 'FREE' && (
                    <span className="bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-xs font-bold px-2.5 py-1 rounded-full uppercase tracking-wider border border-emerald-200 dark:border-emerald-500/20">
                      Active
                    </span>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
              <div className="bg-gray-50 dark:bg-slate-800/50 p-5 rounded-xl border border-gray-100 dark:border-slate-700/50">
                <p className="text-xs text-gray-500 dark:text-slate-400 font-bold uppercase tracking-wider mb-2">Payment Status</p>
                {plan === 'FREE' ? (
                  <p className="font-semibold text-gray-900 dark:text-white">No payment required</p>
                ) : (
                  <div>
                    <p className="font-semibold text-gray-900 dark:text-white mb-1">Payment Received</p>
                    <p className="text-sm text-gray-500 dark:text-slate-400">Your subscription is active and in good standing.</p>
                  </div>
                )}
              </div>
              <div className="bg-gray-50 dark:bg-slate-800/50 p-5 rounded-xl border border-gray-100 dark:border-slate-700/50">
                <p className="text-xs text-gray-500 dark:text-slate-400 font-bold uppercase tracking-wider mb-2">Next Billing Date</p>
                {plan === 'FREE' ? (
            ...
            {plan !== 'FREE' && (
              <div className="mt-8 pt-6 border-t border-gray-100 dark:border-slate-800 relative z-10">
                <h4 className="font-bold text-gray-900 dark:text-white mb-4">Included in your plan</h4>
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-600 dark:text-slate-300">
                  <li className="flex items-center gap-2"><svg className="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg> Unlimited Projects</li>
                  <li className="flex items-center gap-2"><svg className="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg> Unlimited Staff Members</li>
