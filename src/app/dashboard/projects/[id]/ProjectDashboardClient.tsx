...
function DesktopThemeToggle() {
  ...
  if (!mounted) return <div className="w-20 h-9 rounded-full bg-gray-100 dark:bg-slate-800 animate-pulse" />;
  const isDark = resolvedTheme === "dark";
  return (
    <div className="hidden md:flex items-center bg-gray-100 dark:bg-slate-800 rounded-full p-1 border border-gray-200 dark:border-slate-700 gap-0.5">
      <button
        onClick={() => setTheme("light")}
        aria-label="Light mode"
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
          !isDark ? "bg-white text-amber-500 shadow-sm" : "text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200"
        }`}
      >
      ...
      <button
        ...
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
          isDark ? "bg-slate-700 text-slate-200 shadow-sm" : "text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200"
        }`}
      >
...
export default function ProjectDashboardClient({ project, allProjects, currency, userRole, currentUserId, userPermissions = [] }: any) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-brandbg text-gray-900 dark:text-slate-200 pb-24 font-sans transition-colors duration-200">
      
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 sticky top-0 z-30 shadow-sm transition-colors duration-200">
        <div className="flex items-center gap-2 sm:gap-3 relative z-50">
          <Link href="/dashboard/projects" className="w-10 h-10 sm:w-12 sm:h-12 bg-accent-50 dark:bg-accent/10 rounded-xl flex items-center justify-center text-accent-600 dark:text-accent font-bold shrink-0 border border-accent-100 dark:border-accent/20 transition-transform hover:scale-105 active:scale-95">
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <h1 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white line-clamp-1">{project.name}</h1>
                <div className="w-5 h-5 rounded-full bg-gray-100 dark:bg-slate-800 flex items-center justify-center group-hover:bg-gray-200 dark:group-hover:bg-slate-700 transition-colors">
                  <svg className="w-3.5 h-3.5 text-gray-600 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7"></path></svg>
                </div>
              </div>
            </div>
                  ...
                  <div className="max-h-64 overflow-y-auto py-1">
                    ...
                    {allProjects.filter((p: any) => p.name.toLowerCase().includes(projectSearchQuery.toLowerCase())).map((p: any) => (
                      <a 
                        ...
                        className={`block px-5 py-3.5 text-[15px] transition-colors ${p.id === project.id ? 'bg-accent-50 dark:bg-accent/10 text-accent-500 dark:text-accent font-bold' : 'text-gray-700 dark:text-slate-300 font-semibold hover:bg-gray-50 dark:hover:bg-slate-700'}`}
                      >
                        {p.name}
                      </a>
                    ))}
          ...
          {canViewProjectSettings && (
            <div className="relative">
              <button 
                ...
                className="w-10 h-10 rounded-full bg-gray-100 dark:bg-slate-800 flex items-center justify-center text-gray-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors border border-gray-200 dark:border-slate-700 shadow-sm relative z-30"
              >
              ...
              {isEditingBudget ? (
                <div className="flex items-center mt-1 border border-gray-200 dark:border-slate-700 rounded-xl bg-gray-50 dark:bg-slate-800 overflow-hidden focus-within:ring-2 focus-within:ring-accent-500/20 focus-within:border-accent-500 transition-all w-full">
                  ...
                  <input 
                    ...
                    autoFocus
                    className="w-full py-2 pr-3 bg-transparent border-none focus:outline-none focus:ring-0 font-bold text-gray-900 dark:text-white text-xl"
                    placeholder="0.00"
                  />
                </div>
              ) : (
                <p className="text-3xl font-bold text-gray-900 dark:text-white mt-1">{formatCurrency(project.budget || 0, currency)}</p>
              )}
            </div>
              ...
              {isEditingBudget ? (
                <>
                  ...
                  <button 
                    ...
                    disabled={isUpdatingBudget}
                    className="w-10 h-10 rounded-full bg-gray-50 dark:bg-slate-800 flex items-center justify-center text-gray-400 dark:text-slate-500 hover:bg-gray-100 dark:hover:bg-slate-700 hover:text-gray-600 dark:hover:text-slate-300 transition-colors border border-gray-200 dark:border-slate-700 shadow-sm"
                  >
                ...
                project.status !== 'COMPLETED' && (
                  <button 
                    ...
                    className="w-8 h-8 rounded-full bg-gray-50 dark:bg-slate-800 flex items-center justify-center text-gray-500 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700 transition-colors border border-gray-200 dark:border-slate-700"
                  >
      ...
      {/* Error Message Modal */}
      {errorMessage && (
        <div className="fixed inset-0 z-[100] flex flex-col justify-end sm:justify-center items-center bg-white sm:bg-gray-900/40 dark:bg-slate-900 sm:dark:bg-black/60 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-slate-800 sm:border w-full h-full sm:h-auto sm:max-w-sm rounded-none sm:rounded-2xl shadow-none sm:shadow-xl p-6 text-center flex flex-col sm:max-h-[90vh] animate-in zoom-in-95 duration-200">
            <div className="w-12 h-12 rounded-full bg-red-100 dark:bg-red-500/10 text-red-500 mx-auto flex items-center justify-center mb-4">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
            </div>
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{t('error') || "Error"}</h3>
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-6">{errorMessage}</p>
            <button onClick={() => setErrorMessage(null)} className="w-full py-2.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl font-bold hover:bg-gray-800 dark:hover:bg-gray-100 transition-colors">{t('dismiss') || "Dismiss"}</button>
          </div>
        </div>
      )}
      ...
      {expenseToDelete && (
        <div className="fixed inset-0 z-[100] flex flex-col justify-end sm:justify-center items-center bg-white sm:bg-slate-900/40 dark:bg-slate-900 sm:dark:bg-black/60 backdrop-blur-sm p-0 sm:p-4 animate-fade-in" onClick={() => setExpenseToDelete(null)}>
      ...
      {expenseToEdit && (
        <div className="fixed inset-0 z-[100] flex flex-col justify-end sm:justify-center items-center bg-white sm:bg-gray-900/40 dark:bg-slate-900 sm:dark:bg-black/60 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-slate-800 sm:border w-full h-full sm:h-auto sm:max-w-sm rounded-none sm:rounded-2xl shadow-none sm:shadow-xl p-6 flex flex-col sm:max-h-[90vh] animate-in zoom-in-95 duration-200">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">{t('edit_expense') || "Edit Expense"}</h3>
            <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">{t('edit_expense_desc') || "Enter a new amount and reason for this expense."}</p>
            <div className="mb-4">
              <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1.5">{t('new_amount') || "New Amount"}</label>
              <div className="flex items-center border border-gray-200 dark:border-slate-700 rounded-xl bg-gray-50 dark:bg-slate-800 overflow-hidden focus-within:ring-2 focus-within:ring-accent-500/20 focus-within:border-accent-500 focus-within:bg-white dark:focus-within:bg-slate-900 transition-all w-full">
                <span className="pl-3 pr-1 text-gray-400 font-bold">{getCurrencySymbol(currency)}</span>
                <input 
                  type="number"
                  value={editAmountValue}
                  onChange={(e) => setEditAmountValue(e.target.value)}
                  className="w-full py-2.5 pr-3 bg-transparent border-none focus:outline-none focus:ring-0 font-bold text-gray-900 dark:text-white"
                  placeholder="0.00"
                />
              </div>
            </div>
            <div className="mb-6 flex-1 overflow-y-auto">
              <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1.5">{t('reason_for_edit') || "Reason for Edit"}</label>
              <textarea 
                value={editReasonValue}
                onChange={(e) => setEditReasonValue(e.target.value)}
                className="w-full py-2.5 px-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-accent-500/20 focus:border-accent-500 focus:bg-white dark:focus:bg-slate-900 font-medium text-gray-900 dark:text-white resize-none h-24"
                placeholder="Why are you changing this expense?"
              ></textarea>
            </div>
            
            <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-slate-800 mt-auto">
              <button onClick={() => setExpenseToEdit(null)} className="flex-1 py-3 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 rounded-xl font-bold hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors">{t('cancel') || "Cancel"}</button>
              <button 
                onClick={confirmEditExpense} 
                disabled={!editAmountValue || !editReasonValue.trim() || Number(editAmountValue) === expenseToEdit.amount} 
      ...
      {project.status !== 'COMPLETED' && canAddExpense && (
        ...
        >
          <svg className="w-6 h-6 sm:w-7 sm:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4"></path></svg>
          <span className="font-bold text-sm sm:text-base whitespace-nowrap">{t('add_expense') || "Add Expense"}</span>
        </button>
      )}
