...
export default function PaymentRequestClient({ currency, tenantName }: PaymentRequestClientProps) {
  ...
  const handleWhatsAppShare = async () => {
    ...
    const loadingToast = toast.loading("Generating Payment Request Image...");
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        const fileName = `Payment_Request_${project.name.replace(/\s+/g, '_')}.png`;
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          ...
          toast.success("Image downloaded! Please attach it in WhatsApp.", { id: loadingToast });
      ...
      toast.error("Failed to capture image", { id: loadingToast });
      setIsGenerating(false);
    }
  };
      ...
      <div className="max-w-6xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-gray-100 dark:border-slate-700 shadow-sm">
            ...
            <div className="mt-8">
                ...
                <p className="text-xs text-gray-500 text-center mb-4">Click the button below to generate and share this request directly via WhatsApp.</p>
                <button 
        ...
        <div className="space-y-6">
          <div className="bg-[#e7f1eb] dark:bg-emerald-950/20 rounded-2xl p-5 border border-[#d1e6d9] dark:border-emerald-900/30 sticky top-24">
            ...
            <div className="w-full relative rounded-2xl overflow-hidden shadow-sm bg-white border border-gray-200" style={{ height: '460px' }}>
              <div 
                className="absolute top-0 left-0 origin-top-left"
                style={{ transform: 'scale(0.35)', width: '900px' }}
              >
                <div 
                  ref={captureRef} 
                  className="bg-[#fcfdfa] rounded-[24px] p-10 font-sans text-gray-900 border border-[#e4e9e0]" 
                  style={{ width: '900px', height: '1150px', maxWidth: 'none', maxHeight: 'none', letterSpacing: 'normal' }}
                >
                  
                  {/* Header with Logo */}
                  <div className="flex items-center gap-4 mb-10 pb-6 border-b border-gray-200">
                    <img src="/mysitebook-logo-dark.png" alt="MySiteBook" className="h-16" />
                    <div>
                      <h1 className="text-3xl font-bold text-gray-800 tracking-tight" style={{ whiteSpace: 'nowrap' }}>MySiteBook</h1>
                      <p className="text-gray-600 text-xl font-medium">Payment Request</p>
                    </div>
                  </div>

                  <div className="mb-10 text-xl leading-relaxed">
                    <p className="mb-6 font-medium text-gray-800">Hello {project.clientName || 'Client'},</p>
                    <p className="text-gray-700">We have raised a payment request for the project <span className="font-bold text-gray-900">{project.name}</span>.</p>
                  </div>

                  {/* Summary Block */}
                  <div className="space-y-6 text-xl mb-12">
                    <div className="grid grid-cols-[300px_1fr] items-center">
                      <div className="font-bold text-gray-800">Request Amount</div>
                      <div className="font-medium text-gray-900 flex gap-2"><span>:</span> {formatCurrency(Number(requestAmount) || 0, currency)}</div>
                    </div>
                    
                    <div className="grid grid-cols-[300px_1fr] items-center">
                      <div className="font-bold text-gray-800">Due Date</div>
                      <div className="font-medium text-gray-900 flex gap-2"><span>:</span> {dueDate ? dueDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'N/A'}</div>
                    </div>

                    <div className="grid grid-cols-[300px_1fr] items-center">
                      <div className="font-bold text-gray-800">Balance Amount</div>
                      <div className="font-medium text-gray-900 flex gap-2"><span>:</span> {formatCurrency(financials.balanceAmount || 0, currency)}</div>
                    </div>
                  </div>
                  
                  {paymentFor && (
                    <div className="mb-10 text-xl">
                      <p className="font-bold text-gray-800 mb-2">Payment For:</p>
                      <p className="text-gray-700 bg-white p-5 rounded-xl border border-gray-200">{paymentFor}</p>
                    </div>
                  )}
                  ...
                  {notes && (
                    <div className="mb-10 text-xl">
                      ...
                      <p className="text-gray-700 bg-white p-5 rounded-xl border border-gray-200">{notes}</p>
                    </div>
                  )}

                  <div className="mt-auto absolute bottom-10 left-10 right-10">
                    <div className="flex items-center gap-3 bg-white px-6 py-5 rounded-2xl border border-[#e4e9e0] text-[#1d4ed8] font-bold text-xl shadow-sm">
