  ...
  return (
    <aside className="hidden md:flex flex-col w-64 h-screen border-r border-gray-200 bg-white sticky top-0 shrink-0">
      ...
      <div className="p-4 border-t border-gray-100 bg-white space-y-4">
        ...
        <div className="flex bg-gray-100 rounded-xl p-1 border border-gray-200 w-full justify-between">
          <button onClick={() => handleLanguageChange('en')} disabled={isChangingLanguage} className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-colors ${language === 'en' ? 'bg-accent-500 text-white shadow-sm' : 'text-gray-500 hover:bg-white'} ${isChangingLanguage ? 'opacity-50' : ''}`}>EN</button>
          <button onClick={() => handleLanguageChange('ta')} disabled={isChangingLanguage} className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-colors ${language === 'ta' ? 'bg-accent-500 text-white shadow-sm' : 'text-gray-500 hover:bg-white'} ${isChangingLanguage ? 'opacity-50' : ''}`}>தமிழ்</button>
          <button onClick={() => handleLanguageChange('hi')} disabled={isChangingLanguage} className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-colors ${language === 'hi' ? 'bg-accent-500 text-white shadow-sm' : 'text-gray-500 hover:bg-white'} ${isChangingLanguage ? 'opacity-50' : ''}`}>हिंदी</button>
        </div>
        ...
        <button 
          ...
          className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all font-medium text-sm text-gray-600 hover:bg-red-50 hover:text-red-600 border border-transparent outline-none group"
        >
