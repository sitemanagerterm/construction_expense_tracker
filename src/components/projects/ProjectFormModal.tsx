...
export default function ProjectFormModal({ isOpen, onClose, onSuccess, editData }: ProjectFormModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm overflow-y-auto p-4 sm:p-6 transition-all duration-300" onClick={onClose}>
      <div 
        className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-2xl my-8 mx-auto transform transition-all duration-300 animate-in fade-in zoom-in-95 border border-gray-100 dark:border-slate-800"
      ...
      >
        {showUpgradePrompt ? (
          <div className="p-8 sm:p-12 text-center">
            ...
            <h2 className="text-3xl font-black text-gray-900 dark:text-white mb-4">Unlimited Projects with Pro</h2>
            <p className="text-gray-500 dark:text-slate-400 text-lg mb-8 max-w-md mx-auto">
              You have reached the maximum limit of 2 projects on the Free Plan. Upgrade to Pro to unlock unlimited projects and advanced features!
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button onClick={onClose} className="px-6 py-3 rounded-xl font-bold text-gray-500 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors">
                Maybe Later
              </button>
              <a href="/pricing" className="px-8 py-3 bg-accent-500 hover:bg-accent-600 text-white rounded-xl font-bold shadow-lg shadow-accent-500/30 transition-all active:scale-[0.98]">
